...
*Setup instructions for a gitian build of Madoocoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Madoocoin
...
- [Building Madoocoin](#building-madoocoin)
...
Clone the git repositories for madoocoin and gitian,
...
In [doc/release-process.md](release-process.md) in the madoocoin repository under 'Fetch and build inputs'.
...
To build Madoocoin (for Linux and/or Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the madoocoin repository.

Output from `gbuild` will look something like

    Initialized empty Git repository in /home/debian/gitian-builder/inputs/madoocoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/madoocoin-project/madoocoin
...
For example:
```bash
URL=https://github.com/user/madoocoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit madoocoin=${COMMIT} --url madoocoin=${URL} ../madoocoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit bitcoin=${COMMIT} --url madoocoin=${URL} ../madoocoin/contrib/gitian-descriptors/gitian-win.yml
```
...
```bash
    gpg --detach-sign ${VERSION}/${SIGNER}/madoocoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/madoocoin-build.assert
```
...
[madoocoin/gitian.sigs](https://github.com/madoocoin-project/gitian.sigs/) repository, or
